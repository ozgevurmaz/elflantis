...
import { WORKSHOP_AUDIENCE_CONTENT, WORKSHOP_AUDIENCE_ITEMS } from "@/lib/constants";
      ...
      <div className="mx-auto max-w-6xl">
        ...
        >
          <h2 className="font-display text-3xl tracking-tight text-ink-900 sm:text-4xl">
            {WORKSHOP_AUDIENCE_CONTENT.title}
          </h2>
          <p className="mt-4 text-lg leading-relaxed text-ink-700">
            {WORKSHOP_AUDIENCE_CONTENT.description}
          </p>
        </motion.div>

        <div className="mt-12 grid gap-4 sm:grid-cols-2">
          {WORKSHOP_AUDIENCE_ITEMS.map((item, index) => (
            <motion.div
              key={item.id}
              initial="hidden"
              whileInView="visible"
