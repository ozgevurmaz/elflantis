...
import { WORKSHOP_LEARN_CONTENT, WORKSHOP_LEARN_ITEMS } from "@/lib/constants";
    ...
    <section className="px-6 py-20 lg:px-8 lg:py-28">
      ...
      >
        <h2 className="font-display text-3xl tracking-tight text-ink-900 sm:text-4xl">
          {WORKSHOP_LEARN_CONTENT.title}
        </h2>

        <ul className="mt-8 space-y-4">
          {WORKSHOP_LEARN_ITEMS.map((item) => (
            <li key={item.id} className="flex items-start gap-3">
              <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-lavender-100">
                <Sparkles className="h-3.5 w-3.5 text-lavender-600" strokeWidth={2} aria-hidden="true" />
              </span>
